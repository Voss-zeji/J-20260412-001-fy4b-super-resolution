#!/usr/bin/env python3
"""
18_method_gprof_ir - GPROF-IR: 红外降水反演增强

参考论文: arxiv 2605.07167
核心: 单通道红外降水反演增强
...
def parse_args():
    parser = argparse.ArgumentParser(description='GPROF-IR')
...
class IREnhanceBlock(nn.Module):
    ...
    def forward(self, x):
        return x + self.gamma * self.ca(self.conv(x))


class GPROFIRNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=64):
        ...
        self.enhance_blocks = nn.Sequential(
            *[IREnhanceBlock(base_channels) for _ in range(6)]
        )
...
def main():
    ...
    best_psnr, results = 0, {"method": "18_method_gprof_ir", "band": ('Channel07' if args.band == 'CH07' else 'Channel08')}

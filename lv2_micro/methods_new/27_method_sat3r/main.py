#!/usr/bin/env python3
"""
27_method_sat3r - Sat3R: RPC感知深度微调

参考论文: arxiv 2605.07264
核心: RPC感知深度微调DSM重建
...
def parse_args():
    parser = argparse.ArgumentParser(description='Sat3R')
...
class RPCBlock(nn.Module):
    def __init__(self, channels):
        ...
        self.rpc_conv = nn.Conv2d(channels, channels, 1)
        self.gamma = nn.Parameter(torch.zeros(1))

    def forward(self, x):
        rpc_out = self.rpc_conv(F.avg_pool2d(x, 2))
        rpc_out = F.interpolate(rpc_out, x.shape[2:], mode='bilinear', align_corners=False)
        return x + self.gamma * (self.conv(x) + rpc_out)


class Sat3RNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=64):
        ...
        self.rpc_blocks = nn.Sequential(*[RPCBlock(base_channels) for _ in range(6)])
...
def main():
    ...
    best_psnr, results = 0, {"method": "27_method_sat3r", "band": ('Channel07' if args.band == 'CH07' else 'Channel08')}

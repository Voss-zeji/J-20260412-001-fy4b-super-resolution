#!/usr/bin/env python3
"""
22_method_pet_adapter - PET-Adapter: 测试时域适应

参考论文: arxiv 2605.08030
核心: 测试时域适应 + Poisson噪声处理
...
def parse_args():
    parser = argparse.ArgumentParser(description='PET-Adapter')
...
class ResBlock(nn.Module):
    def __init__(self, channels):
        ...
        self.adapter = AdapterBlock(channels)

    def forward(self, x):
        return x + self.adapter(self.conv(x))


class PETAdapterNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=64):
        ...
        self.res_blocks = nn.Sequential(*[ResBlock(base_channels) for _ in range(6)])
...
def main():
    ...
    best_psnr, results = 0, {"method": "22_method_pet_adapter", "band": ('Channel07' if args.band == 'CH07' else 'Channel08')}

#!/usr/bin/env python3
"""
20_method_multispectral_sr - 多光谱卫星图像超分辨率

参考论文: arxiv 2002.00580
核心: 自编码器 + 残差网络的多波段卫星图像SR
...
def parse_args():
    parser = argparse.ArgumentParser(description='MultiSpectral SR')
...
class Encoder(nn.Module):
    def __init__(self, in_channels, base_channels):
        ...
        self.res_blocks = nn.ModuleList([ResidualBlock(base_channels) for _ in range(4)])
...
class MultiSpectralSRNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=64):
        super().__init__()

        self.encoder = Encoder(in_channels, base_channels)
        self.decoder = Decoder(base_channels, out_channels)
...
def main():
    ...
    model = MultiSpectralSRNet().to(device)
    ...
    best_psnr, results = 0, {"method": "20_method_multispectral_sr", "band": ('Channel07' if args.band == 'CH07' else 'Channel08')}

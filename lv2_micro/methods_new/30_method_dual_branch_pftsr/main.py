#!/usr/bin/env python3
"""
30_method_dual_branch_pftsr - Dual-Branch PFTSR
基于 Dual-Branch RS Infrared SR 思想：
- 分支A：PFTSR 空间分支（CNN + CBAM）
- 分支B：频域分支（FFT + 轻量 CNN）
- 融合：通道注意力加权融合
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Dual-Branch PFTSR')
...
class SpatialBranch(nn.Module):
    """空间分支：基于 PFTSR"""
    def __init__(self, in_ch=1, feat=64):
        super().__init__()
        ...
        self.res_blocks = nn.ModuleList([ResidualBlock(feat) for _ in range(4)])
...
class FrequencyBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        real = fft_feat.real
        imag = fft_feat.imag
        fft_concat = torch.cat([real, imag], dim=1)
        fft_processed = self.conv1(fft_concat)
...
class FusionModule(nn.Module):
    ...
    def __init__(self, feat_a=64, feat_b=32, out_feat=64):
        ...
        self.attn = ChannelAttention(out_feat, reduction=8)
    ...
    def forward(self, feat_a, feat_b):
        ...
        merged = self.attn(merged)
        return self.refine(merged)


class DualBranchPFTSR(nn.Module):
    def __init__(self, in_ch=1, out_ch=1, upscale=2):
        super().__init__()
        self.upscale = upscale
        self.spatial_branch = SpatialBranch(in_ch, feat=64)
...
def main():
    ...
    model = DualBranchPFTSR().to(device)
    model_params = sum(p.numel() for p in model.parameters())
    print(f"[DualBranchPFTSR] Params: {model_params:,}")
    ...
    result = {
        "method": "30_method_dual_branch_pftsr",
        "band": args.band,

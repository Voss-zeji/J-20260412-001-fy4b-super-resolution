#!/usr/bin/env python3
"""
29_method_infogeo - InfoGeo: 信息论目标中心跨视图定位

参考论文: arxiv 2605.07099
核心: 信息论目标中心跨视图定位
...
def parse_args():
    parser = argparse.ArgumentParser(description='InfoGeo')
...
class InfoBlock(nn.Module):
    ...
    def forward(self, x):
        return x + self.gamma * self.info_attn(self.conv(x))


class InfoGeoNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=64):
        ...
        self.info_blocks = nn.Sequential(*[InfoBlock(base_channels) for _ in range(6)])
